/* ===============================
   ...
   Servo ON/OFF behavior:
   - ON  → servo to 90° and hold (active / powered)
   - OFF → servo stops completely (PWM off, no holding)
================================ */
    ...
    .then(data => {
        ...
        if (el) {
            if (data.active) {
                ...
                document.getElementById("Relay")?.classList.add("active");
            } else {
                el.textContent = "OFF";
                el.style.color = "#ef4444";
                document.getElementById("Relay")?.classList.remove("active");
            }
        }
    })
...
// Servo ON/OFF toggle – simple ON / OFF display
function toggleServo() {
    ...
    .then(data => {
        ...
        if (data.active) {
            ...
            statusEl.style.color = "#22c55e";      // green = ON / active
            cardEl.classList.add("active");
        } else {
            statusEl.textContent = "OFF";
            statusEl.style.color = "#ef4444";      // red = OFF / deactivated
...
// Update dashboard from /api/sensors
        ...
        document.querySelectorAll(".sensor-card").forEach(card => {
            ...
            if (valueEl) {
                if (name === "Relay") {
                    ...
                    // Show "ON – 90°" when active, "OFF" when inactive
